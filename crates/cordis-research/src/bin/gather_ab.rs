...
use cordis::modal_bank::GATHER_F32;
...
fn armed() -> (CordisEngine, std::sync::mpsc::Sender<CordisCommand>) {
    ...
    for i in 0..VOICES {
        let _ = tx.send(CordisCommand::NoteOn(30 + (i as u8) * 2, 96));
    }
...
fn time_one(eng: &mut CordisEngine, k: Kernel, buf: &mut [f32]) -> f64 {
    k.arm();
