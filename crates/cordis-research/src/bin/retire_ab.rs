...
//!
//!   cargo run --release -p cordis-research --bin retire_ab
//!
...
use cordis::engine::{CordisCommand, CordisEngine};
...
fn render(rel: Option<f64>, secs: f64, score: &[(f64, CordisCommand)]) -> Take {
...
fn main() {
    ...
    let single: Vec<(f64, CordisCommand)> = vec![
        (0.0, CordisCommand::NoteOn(60, 100)),
        (0.5, CordisCommand::NoteOff(60)),
    ];
    ...
    let mut gliss: Vec<(f64, CordisCommand)> = Vec::new();
    for i in 0..30u8 {
        let t = i as f64 * 0.05;
        let note = 48 + i;
        gliss.push((t, CordisCommand::NoteOn(note, 90)));
        gliss.push((t + 0.08, CordisCommand::NoteOff(note)));
    }
