//! In-process cost bench for the cordis engine. Times the DSP directly (no
...
use cordis::soundboard::Soundboard;
use cordis::voice::Voice;
use cordis::engine::{CordisEngine, CordisCommand};
use cordis::patch::factory_presets;
...
fn main() {
    ...
    cordis::denormal::enable_flush_to_zero();
    let sr = 48_000.0f32;
    ...
    println!("\n-- workers, at 31 voices --");
    {
        for &w in &[0usize, 2, 4, 6, 8, 10, 12] {
            ...
            eng.set_workers(w);
            for i in 0..31 {
                tx.send(CordisCommand::NoteOn(48 + (i as u8) * 2, 90)).ok();
            }
            let block = 128usize;
    ...
    // actually costs.
    {
        ...
        let mut buf = vec![0.0f32; block * 2];
        let budget_ms = 1000.0 * block as f64 / sr as f64;
        tx.send(CordisCommand::SustainPedal(true)).ok();
        for _ in 0..8 {
            eng.process_audio(&mut buf, 2);
        }

        tx.send(CordisCommand::NoteOn(48, 90)).ok();
        let t = Instant::now();
        eng.process_audio(&mut buf, 2);
    ...
    for &k in &[1usize, 8, 16, 31] {
        let (mut eng, tx, _mr) = CordisEngine::new_for_plugin(sr);
        tx.send(CordisCommand::LoadPatch(Box::new(preset.clone()))).ok();
        // hold k notes across the tenor/treble
        for i in 0..k { tx.send(CordisCommand::NoteOn(48 + (i as u8) * 2, 90)).ok(); }
        let block = 128usize;
        let mut buf = vec![0.0f32; block * 2];
