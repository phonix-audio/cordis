...
//!
//! Ressac runs six Cordis instances at once and was unplayable live. Each
...
//!
//!   cargo run --release -p cordis-research --bin ensemble_bench
...
fn run_instance(id: usize, capped: bool, sync: Arc<AtomicUsize>) -> (f64, usize, usize) {
    ...
    for b in 0..blocks {
        ...
        if (b * BLOCK) % ((0.3 * SR as f64) as usize / BLOCK * BLOCK) < BLOCK {
            ...
            let _ = tx.send(CordisCommand::NoteOn(n.min(105), 92));
        }
