//! Stereo BBD-style chorus — the secret-sauce width ingredient.
//!
//! Two independent modulated delay lines (one per channel) with their
//! LFOs 180° apart in phase. Asymmetric L/R modulation produces the
//! "huge stereo" perception that blockbuster pads depend on — much
//! more than a centered mono chorus or a static stereo widener.
//!
//! Per `feedback_pad_target_specificity`: stereo width is the #1
//! perceptual ingredient for blockbuster-atmosphere pads. This module
//! is the single biggest sonic gain in the v3 rebuild after the
//! per-voice spread (Phase B).
//!
//! Design notes:
//! - Two modulated delay lines, ~20-40 ms base delay, ±1-5 ms mod depth
//! - Independent LFOs 180° apart: L peaks when R troughs
//! - Linear interpolation on the read head (cheap, sufficient for slow LFOs)
//! - Per-block control-rate LFO update (slow rates don't need per-sample
//!   precision)
//! - Idle bypass: if input is silent for many blocks, skip write+read
