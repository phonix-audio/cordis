...
impl CordisApp {
    ...
    pub fn current_patch(&self) -> CordisPatch {
        self.patch.clone()
    }
    ...
    pub fn set_patch(&mut self, patch: CordisPatch) {
        self.patch = patch;
    ...
    fn send(&mut self, cmd: CordisCommand) {
        self.sync_cooldown = 20;
    ...
    fn draw_keyboard(&mut self, ui: &mut Ui, r: Rect) {
        ...
        for ev in keyboard::draw(ui, keys, &mut self.keys) {
            let cmd = match ev {
                KeyEvent::On { note, velocity } => CordisCommand::NoteOn(note, velocity),
                KeyEvent::Off { note } => CordisCommand::NoteOff(note),
            };
            let _ = self.tx.send(cmd);
        }
    }
    ...
    fn draw_left(&mut self, ui: &mut Ui, r: Rect) {
        ...
        if let Some(v) = self.knob(
            ...
            |v| format!("{v:+.1}c"), CordisCommand::SetTune,
        ) { self.patch.tune = v; }
        y += KNOB + 34.0;
    ...
    fn draw_stage(&mut self, ui: &mut Ui, r: Rect) {
        ...
        if let Some(w) = res.width_changed {
            self.patch.width = w;
            self.send(CordisCommand::SetWidth(w));
        }
    }
    ...
    fn draw_right(&mut self, ui: &mut Ui, r: Rect) {
        ...
        if let Some(v) = self.knob(
            ...
            |v| format!("{:.0}%", v * 100.0), CordisCommand::SetDamper,
        ) { self.patch.damper = v; }
        y += KNOB_BIG + 34.0;
        ...
        if let Some(v) = self.knob(
            ...
            |v| format!("{:.0}%", v * 100.0), CordisCommand::SetGain,
        ) { self.patch.gain = v; }
        y += KNOB + 34.0;
        vu::draw(
            ui,
...
mod tests {
    ...
    #[test]
    fn cordis_ui_builds_and_shows_controls() {
        let (tx, _rx) = std::sync::mpsc::channel();
        let (_mw, mr) = meter_channel::<CordisMeterState>();
        let app = CordisApp::new(tx, mr);
    ...
    #[test]
    fn cordis_gui_mirrors_engine_patch() {
        let (tx, _rx) = std::sync::mpsc::channel();
    ...
    ///   UPDATE_SNAPSHOTS=1 cargo test -p cordis-ui editor_snapshot -- --ignored
    #[test]
    #[ignore = "needs a rendering backend"]
    fn cordis_editor_snapshot() {
        let (tx, _rx) = std::sync::mpsc::channel();
        let (mut w, r) = meter_channel::<CordisMeterState>();
        {
            let s = w.edit();
        ...
        let app = CordisApp::new(tx, r);
        let mut h = harness(app);
        h.run_steps(3);
        h.snapshot("cordis_editor");
    }
