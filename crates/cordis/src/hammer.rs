...
mod tests {
    ...
    fn fit_the_treble_knock() {
        ...
        for db in [-0.35f32, 0.0, 0.25, 0.5, 0.75] {
            ...
            let (mut eng, tx, _m) = crate::engine::CordisEngine::new_for_plugin(sr);
            let mut p = crate::patch::CordisPatch::default();
            p.mechanics = 1.0;
            tx.send(crate::engine::CordisCommand::LoadPatch(Box::new(p))).ok();
            tx.send(crate::engine::CordisCommand::NoteOn(note, 120)).ok();
