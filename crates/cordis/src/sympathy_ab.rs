...
//!
//!   cargo test -p cordis --lib render_the_sympathy_ab -- --ignored --nocapture
...
mod tests {
    ...
    fn render(per_string: bool, secs: f64, score: &[(f64, CordisCommand)]) -> Vec<f32> {
    ...
    fn render_the_sympathy_ab() {
        ...
        let mut chord: Vec<(f64, CordisCommand)> = vec![(0.0, CordisCommand::SustainPedal(true))];
        for n in [40u8, 47, 52, 56, 59] {
            chord.push((0.02, CordisCommand::NoteOn(n, 96)));
        }
        ...
        let mut phrase: Vec<(f64, CordisCommand)> =
            vec![(0.0, CordisCommand::SustainPedal(true))];
        for (i, n) in [52u8, 59, 64, 67, 71, 76].into_iter().enumerate() {
            let t = 0.05 + i as f64 * 0.55;
            phrase.push((t, CordisCommand::NoteOn(n, 92)));
            phrase.push((t + 0.45, CordisCommand::NoteOff(n)));
        }
