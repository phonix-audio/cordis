...
pub struct CordisMeterState {
    pub peak_l: f32,
    pub peak_r: f32,
    pub active_voices: u8,
    pub patch_snapshot: Option<CordisPatch>,
...
pub struct CordisEngine {
    ...
    held: Vec<u8>,
    sustain: bool,
    command_rx: mpsc::Receiver<CordisCommand>,
    meter_writer: Writer<CordisMeterState>,
    meter_shadow: CordisMeterState,
    meter_counter: usize,
    peak_l: f32,
    peak_r: f32,
    patch_dirty: bool,
...
impl CordisEngine {
    pub fn new(
        sr: f32,
        rx: mpsc::Receiver<CordisCommand>,
        meter_writer: Writer<CordisMeterState>,
    ) -> Self {
        let patch = CordisPatch::default();
        let board = Soundboard::new(sr, patch.width as f64);
        let mut eng = CordisEngine {
            strike_seq: 0,
            patch,
            sample_rate: sr,
            ...
            command_rx: rx,
            meter_writer,
            meter_shadow: CordisMeterState::default(),
            meter_counter: 0,
            peak_l: 0.0,
            peak_r: 0.0,
            patch_dirty: true,
    ...
    pub fn new_for_plugin(
        sr: f32,
    ) -> (Self, mpsc::Sender<CordisCommand>, SharedReader<CordisMeterState>) {
    ...
    fn drain_commands(&mut self) {
        ...
        while let Ok(cmd) = self.command_rx.try_recv() {
            match cmd {
                ...
                CordisCommand::SetVoicing(x) => self.patch.voicing = x.clamp(0.0, 1.0),
                CordisCommand::SetUnisonDetune(x) => {
                    self.patch.unison_detune = x.clamp(0.0, 20.0)
                }
                CordisCommand::SetWidth(x) => {
                ...
                CordisCommand::SetMechanics(x) => self.patch.mechanics = x.clamp(0.0, 1.0),
                CordisCommand::SetReleaseNoise(x) => {
                    self.patch.release_noise = x.clamp(0.0, 1.0)
                }
                CordisCommand::SetTune(x) => self.patch.tune = x.clamp(-100.0, 100.0),
                CordisCommand::SetGain(x) => self.patch.gain = x.clamp(0.0, 2.0),
                CordisCommand::LoadPatch(p) => {
...
mod tests {
    ...
    fn probe_onset_click_source() {
        ...
        for note in [48u8, 60, 72] {
            for (lbl, mech) in [("mech ON ", 0.5f32), ("mech OFF", 0.0)] {
                let sig = CordisEngine::render_note_with(SR, note, 100, 0.4, |p| p.mechanics = mech);
    ...
    fn a_full_chord_stays_finite() {
        ...
        for count in [1usize, 4, 16, 32, 64, 96, MAX_VOICES] {
            let (mut eng, tx, _mr) = CordisEngine::new_for_plugin(SR);
            let _ = tx.send(CordisCommand::SustainPedal(true));
            for i in 0..count {
                ...
                let _ = tx.send(CordisCommand::NoteOn(note, 110));
            }
            let mut buf = vec![0.0f32; 256 * 2];
            let mut pk = 0.0f32;
    ...
    fn the_sustain_pedal_holds_the_note() {
        let (mut eng, tx, _mr) = CordisEngine::new_for_plugin(SR);
        let mut buf = vec![0.0f32; 256 * 2];
        let mut run = |eng: &mut CordisEngine, secs: f32| -> f32 {
            let mut p = 0.0f32;
    ...
    fn the_meter_reports_audible_notes_and_the_pedal() {
        let (mut eng, tx, mut mr) = CordisEngine::new_for_plugin(SR);
        let mut buf = vec![0.0f32; 256 * 2];
        let mut run = |eng: &mut CordisEngine, secs: f32| {
            for _ in 0..((SR * secs) as usize / 256) {
                buf.iter_mut().for_each(|x| *x = 0.0);
                eng.process_audio(&mut buf, 2);
            }
        };
        let read = |mr: &mut SharedReader<CordisMeterState>| {
        ...
        let _ = tx.send(CordisCommand::SustainPedal(false));
        let _ = tx.send(CordisCommand::AllNotesOff);
        run(&mut eng, 0.3);
        let m = read(&mut mr);
    ...
    fn shedding_a_string_does_not_click() {
        let biggest_step = |shed: bool| -> f32 {
            ...
            let _ = tx.send(CordisCommand::NoteOn(52, 100));
            let _ = tx.send(CordisCommand::NoteOn(59, 100));
            let _ = tx.send(CordisCommand::NoteOn(64, 100));
            let mut buf = vec![0.0f32; 128 * 2];
            for _ in 0..30 {
                eng.process_audio(&mut buf, 2);
            }
    ...
    fn the_pool_renders_what_one_thread_renders() {
        let render = |workers: usize| -> Vec<f32> {
            ...
            for n in [40u8, 47, 52, 59, 64, 69, 76, 83] {
                let _ = tx.send(CordisCommand::NoteOn(n, 96));
            }
            let mut out = Vec::new();
            let mut buf = vec![0.0f32; 128 * 2];
            for _ in 0..120 {
...
fn the_pedal_is_audible_on_the_path_the_instrument_takes() {
    ...
    let take = |pedal: bool| -> (Vec<f32>, usize) {
        ...
        let _ = tx.send(CordisCommand::NoteOn(STRUCK, 84));
        let mut x = Vec::new();
        let mut buf = vec![0.0f32; 256 * 2];
...
fn flipping_the_decoupled_board_off_restores_the_exact_engine() {
    ...
    let render = |flip: bool| -> Vec<f32> {
        ...
        let _ = tx.send(CordisCommand::NoteOn(60, 96));
        let mut out = Vec::new();
        let mut buf = vec![0.0f32; 256 * 2];
        for _ in 0..((sr as usize / 2) / 256) {
            buf.iter_mut().for_each(|v| *v = 0.0);
...
fn the_pedal_wakes_the_strings_that_answer() {
    ...
    let (mut eng, tx, _mr) = CordisEngine::new_for_plugin(sr);
    let _ = tx.send(CordisCommand::SustainPedal(true));
    let _ = tx.send(CordisCommand::NoteOn(48, 100));
...
fn the_answering_strings_are_worth_their_slots() {
    ...
    let (mut eng, tx, _mr) = CordisEngine::new_for_plugin(sr);
    let _ = tx.send(CordisCommand::SustainPedal(true));
    let _ = tx.send(CordisCommand::NoteOn(48, 110));
    let mut buf = vec![0.0f32; 512 * 2];
    for _ in 0..47 {
        buf.fill(0.0);
        eng.process_audio(&mut buf, 2);
    }
    let _ = tx.send(CordisCommand::NoteOff(48));
    for _ in 0..94 {
        buf.fill(0.0);
        eng.process_audio(&mut buf, 2);
    }
...
fn print_the_balance_across_the_compass() {
    ...
    for note in (21u8..=105).step_by(6) {
        let out = CordisEngine::render_note_for_analysis(sr, note, 90, 1.5);
...
fn print_whether_partials_are_born_weak_or_die_young() {
    ...
    for note in [45u8, 57, 69, 81] {
        let out = CordisEngine::render_note_for_analysis(sr, note, 100, 3.0);
...
fn audit_many_strings_at_once() {
    ...
    let apart = {
        ...
        for n in chord.iter() {
            let one = CordisEngine::render_note_for_analysis(sr, *n, 100, secs);
...
fn a_stolen_voice_is_the_faintest_one() {
    ...
    for i in 0..MAX_VOICES {
        let _ = tx.send(CordisCommand::NoteOn(21 + (i % 87) as u8, 100));
    }
    let mut buf = vec![0.0f32; 512 * 2];
    for _ in 0..60 {
        buf.fill(0.0);
...
fn a_struck_note_is_loudest_when_it_is_struck() {
    ...
    for (note, latest_ms) in [(45u8, 90.0f64), (57, 40.0), (69, 30.0), (81, 25.0)] {
        let x = CordisEngine::render_note_for_analysis(sr, note, 100, 1.0);
...
fn dump_contact_force() {
    ...
    for note in [45u8, 57, 69, 81] {
        let (mut eng, tx, _mr) = CordisEngine::new_for_plugin(sr);
        let _ = tx.send(CordisCommand::NoteOn(note, 100));
        ...
        let path = format!("/tmp/cordis_force_{note}.txt");
...
fn dump_notes_for_analysis() {
    ...
    for (tag, voicing) in [("soft", 0.0f32), ("mid", 0.5), ("hard", 1.0)] {
        let out = CordisEngine::render_note_with(sr, 69, 100, 3.0, |p| p.voicing = voicing);
        ...
        let path = format!("/tmp/cordis_felt_{tag}.wav");
    ...
    for note in [33u8, 45, 57, 69, 81, 93] {
        ...
        let path = format!("/tmp/cordis_note_{note}.wav");
        let mut w = hound::WavWriter::create(&path, spec).expect("wav");
        for s in out.iter() {
            w.write_sample(*s).unwrap();
        }
...
fn audit_the_level_across_the_compass() {
    ...
    for note in (21u8..=108).step_by(3) {
        let x = CordisEngine::render_note_for_analysis(sr, note, 100, 1.5);
...
fn the_repeated_note_of_the_raindrop_stays_finite() {
    ...
    for b in 0..blocks {
        ...
        while next < EVENTS.len() && EVENTS[next].0 <= t {
            ...
            let _ = tx.send(if on {
                CordisCommand::NoteOn(note, vel)
            } else {
                CordisCommand::NoteOff(note)
            });
            next += 1;
        }
        buf.iter_mut().for_each(|x| *x = 0.0);
...
fn audit_the_excitation_spectrum() {
    ...
    for note in [48u8, 60, 72] {
        ...
        for (label, sig) in [("cordis", &fh), ("chevalet", &fb)] {
...
fn print_where_it_stops_being_a_piano() {
    ...
    for note in (21u8..=105).step_by(6) {
        let out = CordisEngine::render_note_for_analysis(sr, note, 100, 3.5);
        // NOT chunks_exact(2): `render_note_for_analysis` already returns mono.
        // Pairing its samples decimates by two, which halves the length and puts
        // every frequency an octave up — it read A4 as 879 Hz and made the tone
        // look as though it had almost no partials.
...
fn bench_realtime_factor() {
    ...
    eprintln!(
        "cordis 10 voix : {:.3}s d'audio en {:.3}s => {:.1}x temps reel",
        secs,
        el,
        secs / el
    );
...
fn print_the_pedalled_add_to_calibrate_the_halo() {
    ...
    let witness_db = |per_string: bool, pedal: bool, gain: Option<f32>, pair: (u8, u8)| -> f64 {
        ...
        let _ = tx.send(CordisCommand::NoteOn(pair.0, 84));
...
fn print_how_loud_the_voices_in_a_glissando_are() {
    ...
    for b in 0..((sr as usize * 4) / 256) {
        ...
        while next < 30 && (next as f64) * 0.05 <= t {
            let _ = tx.send(CordisCommand::NoteOn(48 + next as u8, 90));
            if next > 0 {
                let _ = tx.send(CordisCommand::NoteOff(48 + next as u8 - 1));
            }
            next += 1;
        }
        buf.iter_mut().for_each(|v| *v = 0.0);
...
fn audit_one_note_envelope() {
    let sr = 48_000.0f32;
    for note in [45u8, 69, 81] {
        let x = CordisEngine::render_note_for_analysis(sr, note, 100, 30.0);
...
fn audit_the_unison_and_the_double_decay() {
    ...
    for note in [57u8, 69, 81] {
        ...
        for cents in [0.0f32, 0.3, 1.0, 2.0, 4.0] {
            let x = CordisEngine::render_note_with(sr, note, 100, 9.0, |p| {
...
fn sweep_to(path: &str, mechanism: bool) {
    ...
    let mut run = |eng: &mut CordisEngine, out: &mut Vec<f32>, n: usize| {
        for _ in 0..(n / 64) {
            buf.fill(0.0);
            eng.process_audio(&mut buf, 2);
            out.extend_from_slice(&buf);
    ...
    for pedal in [false, true] {
        let _ = tx.send(CordisCommand::SustainPedal(pedal));
        for vel in [24u8, 52, 84, 112] {
            for note in 21u8..=108 {
                let _ = tx.send(CordisCommand::NoteOn(note, vel));
                run(&mut eng, &mut out, hold);
                let _ = tx.send(CordisCommand::NoteOff(note));
    ...
    for duplex in [true, false] {
        ...
        for b in 0..((sr as usize * 5) / 256) {
            ...
            while next < notes.len() && next as f64 * 0.45 <= t {
                let _ = tx.send(CordisCommand::NoteOn(notes[next], 92));
                if next > 0 {
                    let _ = tx.send(CordisCommand::NoteOff(notes[next - 1]));
                }
                next += 1;
            }
            buf.iter_mut().for_each(|x| *x = 0.0);
...
fn audit_the_low_thud_under_every_note() {
    ...
    for note in [48u8, 60, 72, 84, 96, 105] {
        let x = CordisEngine::render_note_with(sr, note, 100, 0.06, |p| p.mechanics = 0.0);
        let mut buf: Vec<Complex<f32>> = (0..N)
            .map(|k| {
                let w = 0.5 - 0.5 * (std::f64::consts::TAU * k as f64 / N as f64).cos();
                Complex { re: x.get(k).copied().unwrap_or(0.0) * w as f32, im: 0.0 }
...
fn audit_the_decay_against_the_coupling() {
    ...
    for note in (21u8..=105).step_by(6) {
        ...
        let x = CordisEngine::render_note_for_analysis(sr, note, 100, 4.0);
        let blk = (0.01 * sr) as usize;
        let env: Vec<f64> = x
            .chunks(blk)
            .map(|c| {
...
fn audit_the_buzz_between_the_partials() {
    ...
    for note in [28u8, 33, 40, 45, 52, 57, 64, 69, 76] {
        let d = crate::scale::design(note);
        let x = CordisEngine::render_note_with(sr, note, 100, 1.2, |p| p.mechanics = 0.0);
...
fn audit_the_treble_content_note_by_note() {
    ...
    for note in 21u8..=44 {
        let x = CordisEngine::render_note_with(sr, note, 24, 0.25, |p| p.mechanics = 0.0);
...
fn audit_the_phantom_partials_against_conklin() {
    ...
    for note in [28u8, 36, 45, 52, 60] {
        let d = crate::scale::design(note);
        let x = CordisEngine::render_note_with(sr, note, 112, 2.0, |p| p.mechanics = 0.0);
...
fn audit_what_the_sympathetic_strings_add() {
    ...
    for (note, witness) in [(26u8, 32u8), (33, 39), (40, 46), (52, 58)] {
        ...
        let level = |pedal: bool| -> f64 {
            ...
            let _ = tx.send(CordisCommand::NoteOn(note, 84));
            let mut buf = vec![0.0f32; 256 * 2];
            let mut x: Vec<f32> = Vec::new();
    ...
    for mode in ['A', 'B', 'C'] {
        ...
        for &note in &notes {
            if mode == 'B' || eng_tx.is_none() {
                let (e, t, _m) = CordisEngine::new_for_plugin(sr);
                let mut p = CordisPatch::default();
                p.mechanics = 0.0;
                let _ = t.send(CordisCommand::LoadPatch(Box::new(p)));
                eng_tx = Some((e, t));
            }
            ...
            let _ = tx.send(CordisCommand::NoteOn(note, 24));
            let mut x: Vec<f32> = Vec::new();
            let mut buf = vec![0.0f32; 128 * 2];
            while x.len() < step {
                if x.len() >= hold {
...
fn audit_the_tenor_fundamental_vs_tilt() {
    ...
    for note in [48u8, 55, 60] {
        for depth in [776u32, 650, 550, 400] {
            ...
            let x = CordisEngine::render_note_with(sr, note, 80, 0.2, |p| p.mechanics = 0.0);
...
fn audit_what_makes_the_hf() {
    ...
    for note in [60u8, 72, 84] {
        ...
        for ten in [true, false] {
            ...
            let x = CordisEngine::render_note_with(sr, note, 90, 0.3, |p| p.mechanics = 0.0);
...
fn audit_the_damper_strips_hf() {
    ...
    for release_ms in [10_000u32, 120, 80] {
        let (mut eng, tx, _m) = CordisEngine::new_for_plugin(sr);
        { let mut p = CordisPatch::default(); p.mechanics = 0.0; let _ = tx.send(CordisCommand::LoadPatch(Box::new(p))); }
        let _ = tx.send(CordisCommand::NoteOn(72, 90));
        let mut x: Vec<f32> = Vec::new();
        let mut buf = vec![0.0f32; 128*2];
        let rel = (release_ms as f32/1000.0*sr) as usize;
        let mut sent=false;
        while x.len() < (0.30*sr) as usize {
            if !sent && x.len() >= rel { let _ = tx.send(CordisCommand::NoteOff(72)); sent=true; }
...
fn audit_the_hf_decay_of_a_mid_note() {
    ...
    let x = CordisEngine::render_note_for_analysis(sr, note, 90, 1.0);
    const W: usize = 4096;
    let mut planner = FftPlanner::new();
    let fft = planner.plan_fft_forward(W);
...
fn audit_the_attack_shape() {
    ...
    for note in [24u8, 48, 72, 96] {
        ...
        for direct in [true, false] {
            crate::voice::DIRECT_BLOW.store(direct, Relaxed);
            let x = CordisEngine::render_note_with(sr, note, 48, 0.30, |p| p.mechanics = 0.0);
    ...
    let x = CordisEngine::render_note_with(sr, 24, 48, 0.10, |p| p.mechanics = 0.0);
        ...
        while out.len() < (1.9*sr) as usize * 2 {
            ...
            while ei < events.len() && events[ei].0 <= tsec {
                ...
                if !dropped && !dropped_off {
                    if on { let _=tx.send(CordisCommand::NoteOn(n,70)); }
                    else { let _=tx.send(CordisCommand::NoteOff(n)); }
                }
                ei += 1;
            }
...
fn probes() {
    ...
    for (nm, depth) in [("FSD_F#2_tilt0776", 776u32), ("FSD_F#2_tilt0500", 500)] {
        ...
        let (mut eng, tx, _m) = CordisEngine::new_for_plugin(sr);
        { let mut p = CordisPatch::default(); p.mechanics=0.0; let _=tx.send(CordisCommand::LoadPatch(Box::new(p))); }
        let _=tx.send(CordisCommand::NoteOn(42,70));
        let mut out: Vec<f32>=Vec::new(); let mut buf=vec![0.0f32;128*2];
        while out.len()<(1.6*sr) as usize*2 {
            if out.len()==(0.6*sr) as usize*2 { let _=tx.send(CordisCommand::NoteOff(42)); }
            buf.fill(0.0); eng.process_audio(&mut buf,2); out.extend_from_slice(&buf);
        }
    ...
    for note in [41u8, 42, 43] {
        let (mut eng, tx, _m) = CordisEngine::new_for_plugin(sr);
        { let mut p = CordisPatch::default(); p.mechanics = 0.0; let _ = tx.send(CordisCommand::LoadPatch(Box::new(p))); }
        let _ = tx.send(CordisCommand::NoteOn(note, 70));
        let mut out: Vec<f32> = Vec::new(); let mut buf = vec![0.0f32; 128*2];
        while out.len() < (1.6*sr) as usize*2 {
            if out.len() == (0.6*sr) as usize*2 { let _ = tx.send(CordisCommand::NoteOff(note)); }
            buf.fill(0.0); eng.process_audio(&mut buf, 2); out.extend_from_slice(&buf);
        }
        let peak=out.iter().fold(0.0f32,|a,v|a.max(v.abs())).max(1e-9);
    ...
    for (name, depth) in [("ACC1_tilt_0776", 776u32), ("ACC2_tilt_0550", 550)] {
        ...
        for grp in seq {
            for &n in grp { let _ = tx.send(CordisCommand::NoteOn(n, 70)); }
            let target = out.len() + step*2;
            let off = out.len() + (step*2*3/4);
            let mut done=false;
            while out.len() < target {
                if !done && out.len()>=off { for &n in grp { let _ = tx.send(CordisCommand::NoteOff(n)); } done=true; }
                buf.fill(0.0); eng.process_audio(&mut buf, 2); out.extend_from_slice(&buf);
            }
        }
        let peak = out.iter().fold(0.0f32,|a,v|a.max(v.abs())).max(1e-9);
    ...
    eprintln!("  G : le coup du cordis transmis au chevalet PENDANT le contact (G1 avec, G2 sans)");
...
fn audit_does_the_blow_reach_the_bridge() {
    ...
    for note in [21u8, 45, 69] {
        ...
        eprintln!("     t(ms)    F cordis    F·a attendu    F chevalet    rapport");
        let mut i = 0usize;
        loop {
            let f = v.tick(0.0, 0.0);
            let fh = v.contact_force();
...
fn the_whole_compass_stays_inside_full_scale() {
    ...
    for note in (21u8..=108).step_by(7) {
        for vel in [40u8, 80, 127] {
            let x = CordisEngine::render_note_for_analysis(sr, note, vel, 0.6);
            let peak = x.iter().fold(0.0f32, |a, v| a.max(v.abs())) as f64;
