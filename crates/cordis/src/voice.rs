...
/// note's own peak (`cargo run -p cordis-research --bin retire_ab`: one damped
...
///
/// `CordisEngine::set_retire_rel(0.0)` puts the old rule back.
...
impl Voice {
    ...
    fn build_strings_with(
        ...
        for i in 0..n {
            ...
            // — see `CordisEngine::set_decoupled`) that path is gone, so its
