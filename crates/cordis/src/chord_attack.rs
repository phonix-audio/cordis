...
//!
//!   cargo test -p cordis --lib render_the_chord_attack -- --ignored --nocapture
...
mod tests {
    ...
    fn render(notes: &[u8], secs: f64) -> Vec<f32> {
        ...
        let _ = tx.send(CordisCommand::LoadPatch(Box::new(preset)));
        eng.set_workers(0);

        // Struck on the first block, so the onset sits at a known sample.
        for &n in notes {
            let _ = tx.send(CordisCommand::NoteOn(n, 112));
        }
        let block = 64usize;
    ...
    fn render_the_plate_damping_ab() {
        ...
        const CHORD: [u8; 5] = [40, 47, 52, 56, 59];
        let phrase: Vec<(f64, CordisCommand)> = {
            let mut v = Vec::new();
            for (i, n) in [52u8, 59, 64, 67, 71, 76].into_iter().enumerate() {
                let t = 0.05 + i as f64 * 0.45;
                v.push((t, CordisCommand::NoteOn(n, 108)));
                v.push((t + 0.38, CordisCommand::NoteOff(n)));
            }
            v
        };

        let scored = |secs: f64, score: &[(f64, CordisCommand)]| -> Vec<f32> {
            let preset = factory_presets()
                .into_iter()
                .find(|p| p.name.contains("Close Mics"))
                .expect("the Close Mics preset");
        ...
        let chord_score: Vec<(f64, CordisCommand)> = CHORD
            .iter()
            .map(|&n| (0.02, CordisCommand::NoteOn(n, 112)))
            .collect();
    ...
    fn render_square(tag: &str, chord: &[u8], vel: u8) {
        let render = |pedal: bool, halo_gain: Option<f32>, per_string: bool| -> Vec<f32> {
            ...
            for &n in chord {
                let _ = tx.send(CordisCommand::NoteOn(n, vel));
            }
    ...
    fn render_the_pp_mechanics_ab() {
        let pair = |notes: &[u8], vel: u8, secs: f64| -> Vec<f32> {
            let take = |mech: bool| -> Vec<f32> {
                ...
                let _ = tx.send(CordisCommand::LoadPatch(Box::new(preset)));
                if !mech {
                    let _ = tx.send(CordisCommand::SetMechanics(0.0));
                }
                eng.set_workers(0);
                for &n in notes {
                    let _ = tx.send(CordisCommand::NoteOn(n, vel));
