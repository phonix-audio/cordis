//! PhonixSeq data model — Session, Track, Pattern, MIDI notes.
...
/// The VST3 class id of the Cordis plugin, as 32 ASCII hex characters.
///
/// A legacy `.aeth` names its piano `"Marteau"`, a tag that no longer exists —
/// the plugin ships as Cordis, by Phonix Audio, since the rename. The class id
/// is what the track becomes instead, and it is how the host finds the bundle
/// wherever the user installed it, so it is the CURRENT id that goes in: an old
/// track resolved against the old id would find nothing installed.
pub const CORDIS_CLASS_ID_HEX: &str = "5078436F726469735069616E6F303031";

/// The class id the same piano shipped under before the rename, still written
/// into every session and `.vstpreset` saved while it was "Phonix Marteau".
///
/// Kept so a track carrying it can be recognised and rewritten; the mapping
/// itself lives in `phonix_preset::devices::RENAMED_DEVICES`, which is what
/// export and import read.
pub const MARTEAU_LEGACY_CLASS_ID_HEX: &str = "50784D6172746561754D6F646C303031";

/// The VST3 class id of the Souffle plugin, as 32 ASCII hex characters.
///
/// Same story as the piano above: a legacy `.aeth` names its winds `"Souffle"`,
/// a tag that no longer exists, and the class id is what the track becomes.
pub const SOUFFLE_CLASS_ID_HEX: &str = "5078536F7566666C5761766730303161";

/// The VST3 class id of the Atmosphera plugin, as 32 ASCII hex characters.
///
/// Same story again: a legacy `.aeth` names its ambient pads `"Atmosphera"`, a
/// tag that no longer exists, and the class id is what the track becomes.
pub const ATMOSPHERA_CLASS_ID_HEX: &str = "507841746D6F73706865726150616431";

/// The VST3 class id of the TechnoKick plugin, as 32 ASCII hex characters.
///
/// Same story again: a legacy `.aeth` names its kick designer `"TechnoKick"`, a
/// tag that no longer exists, and the class id is what the track becomes.
pub const TECHNO_KICK_CLASS_ID_HEX: &str = "5078546563686E6F4B69636B30303031";

/// The VST3 class id of the Spectral plugin, as 32 ASCII hex characters.
///
/// Same story again: a legacy `.aeth` names its resynthesis oscillator
/// `"Spectral"`, a tag that no longer exists, and the class id is what the
/// track becomes.
pub const SPECTRAL_CLASS_ID_HEX: &str = "5078537065637472616C526573796E31";

/// The VST3 class id of the VCS3 plugin, as 32 ASCII hex characters.
///
/// Same story again: a legacy `.aeth` names its patchable modular `"Vcs3"`, a
/// tag that no longer exists, and the class id is what the track becomes.
pub const VCS3_CLASS_ID_HEX: &str = "507856637333506174636853796E3031";

/// The VST3 class id of the Magma plugin, as 32 ASCII hex characters.
///
/// Same story again: a legacy `.aeth` names its techno bass `"Magma"`, a tag
/// that no longer exists, and the class id is what the track becomes.
/// `PxMagmaBassSyn01`, byte order preserved.
pub const MAGMA_CLASS_ID_HEX: &str = "50784D61676D614261737353796E3031";

/// The VST3 class id of the Harmonizer plugin, as 32 ASCII hex characters.
///
/// Same story again: a legacy `.aeth` names its vocal harmonizer
/// `"Harmonizer"`, a tag that no longer exists, and the class id is what the
/// track becomes. `PxHarmonizer0001`, byte order preserved.
pub const HARMONIZER_CLASS_ID_HEX: &str = "50784861726D6F6E697A657230303031";

/// The VST3 class id of the Phonix Orgue plugin, as 32 ASCII hex characters.
///
/// Same story again: a legacy `.aeth` names its tonewheel organ `"Orgue"`, a
/// tag that no longer exists, and the class id is what the track becomes.
/// `PxOrgueTonewh001`, byte order preserved.
pub const ORGUE_CLASS_ID_HEX: &str = "50784F72677565546F6E657768303031";

/// The VST3 class id of the Phonix Grain plugin, as 32 ASCII hex characters.
///
/// Same story again: a legacy `.aeth` names its granular synth `"Grain"`, a
/// tag that no longer exists, and the class id is what the track becomes.
/// `PxGrainGranul001`, byte order preserved.
pub const GRAIN_CLASS_ID_HEX: &str = "5078477261696E4772616E756C303031";

/// The VST3 class id of the Phonix Polaris plugin, as 32 ASCII hex characters.
///
/// Same story again: a legacy `.aeth` names its virtual analog `"Polaris"` — or
/// `"NordLead"`, the name it shipped under before the rename — and neither tag
/// exists any more, so the class id is what the track becomes.
///
/// `PxNordLeadSyn001`, byte order preserved. The id says NordLead and the
/// plugin is called Polaris: the instrument was renamed long after the id was
/// frozen, and regenerating it would orphan every session and preset pointing
/// at it. It is a wire format, not a label.
pub const POLARIS_CLASS_ID_HEX: &str = "50784E6F72644C65616453796E303031";

/// The VST3 class id of the Phonix Mellotron plugin, as 32 ASCII hex characters.
///
/// Same story again: a legacy `.aeth` names its tape-replay keyboard
/// `"Mellotron"`, a tag that no longer exists, and the class id is what the
/// track becomes. `PxMellotronTape1`, byte order preserved.
pub const MELLOTRON_CLASS_ID_HEX: &str = "50784D656C6C6F74726F6E5461706531";

/// The VST3 class id of the Phonix Sampler plugin, as 32 ASCII hex characters.
///
/// A `.aeth` written before the extraction names its track engine
/// `"Sampler"`, a tag that no longer exists, and the class id is what the
/// track becomes. `PxSamplerZone001`, byte order preserved.
pub const SAMPLER_CLASS_ID_HEX: &str = "507853616D706C65725A6F6E65303031";

/// The VST3 class id of the Phonix Aurora plugin, as 32 ASCII hex characters.
///
/// Same story again: a legacy `.aeth` names its cinematic pad synth
/// `"Aurora"`, a tag that no longer exists, and the class id is what the
/// track becomes. `PxAuroraPadSyn01`, byte order preserved.
pub const AURORA_CLASS_ID_HEX: &str = "50784175726F726150616453796E3031";

/// The VST3 class id of the Phonix TB-303 plugin, as 32 ASCII hex characters.
///
/// Same story again: a legacy `.aeth` names its acid bassline synth `"Tb303"`,
/// a tag that no longer exists, and the class id is what the track becomes.
/// `PxTb303AcidSyn01`, byte order preserved.
pub const TB303_CLASS_ID_HEX: &str = "507854623330334163696453796E3031";

/// The VST3 class id of the Phonix Nebula plugin, as 32 ASCII hex characters.
///
/// Same story again: a legacy `.aeth` names its generative texture engine
/// `"Nebula"`, a tag that no longer exists, and the class id is what the track
/// becomes. `PxNebulaLayers01`, byte order preserved.
pub const NEBULA_CLASS_ID_HEX: &str = "50784E6562756C614C61796572733031";

/// The VST3 class id of the Phonix Solstice plugin, as 32 ASCII hex characters.
///
/// Same story again: a legacy `.aeth` names its hybrid FM+analog synth
/// `"Solstice"`, a tag that no longer exists, and the class id is what the track
/// becomes. `PxSolsticeSyn001`, byte order preserved.
pub const SOLSTICE_CLASS_ID_HEX: &str = "5078536F6C737469636553796E303031";

/// The VST3 class id of the Phonix VP-330 plugin, as 32 ASCII hex characters.
///
/// Same story again: a legacy `.aeth` names its vocoder / string ensemble
/// `"VP330"`, a tag that no longer exists, and the class id is what the track
/// becomes. `PxVP330Vocoder01`, byte order preserved.
pub const VP330_CLASS_ID_HEX: &str = "50785650333330566F636F6465723031";

/// The VST3 class id of the Phonix Strata plugin, as 32 ASCII hex characters.
///
/// Same story again: a legacy `.aeth` names its four-layer synth `"Strata"`, a
/// tag that no longer exists, and the class id is what the track becomes.
/// `PxStrataSyn00001`, byte order preserved.
pub const STRATA_CLASS_ID_HEX: &str = "507853747261746153796E3030303031";
...
pub enum EngineTag {
    Loquace,
    Singer,
    // Polaris left this crate: it is the `Polaris` VST3, in its own
    // repository. A legacy `.aeth` naming it — under either `Polaris` or the
    // older `NordLead` — is rewritten by `de_engine_tag_compat` below into a
    // `Vst3` tag carrying `POLARIS_CLASS_ID_HEX`.
    // VP330 left this crate: it is the `Phonix VP-330` VST3, in its own
    // repository. A legacy `.aeth` naming it is rewritten by
    // `de_engine_tag_compat` below into a `Vst3` tag carrying
    // `VP330_CLASS_ID_HEX`.
    Drum,
    // Solstice left this crate: it is the `Phonix Solstice` VST3, in its own
    // repository. A legacy `.aeth` naming it is rewritten by
    // `de_engine_tag_compat` below into a `Vst3` tag carrying
    // `SOLSTICE_CLASS_ID_HEX`.
    // Strata left this crate: it is the `Phonix Strata` VST3, in its own
    // repository. A legacy `.aeth` naming it is rewritten by
    // `de_engine_tag_compat` below into a `Vst3` tag carrying
    // `STRATA_CLASS_ID_HEX`.
    // Harmonizer left this crate: it is the `Phonix Harmonizer` VST3, in its
    // own repository. A legacy `.aeth` naming it is rewritten by
    // `de_engine_tag_compat` below into a `Vst3` tag carrying
    // `HARMONIZER_CLASS_ID_HEX`.
    Guitar,
    Bass,
    // Tb303 left this crate: it is the `Phonix TB-303` VST3, in its own
    // repository. Legacy sessions naming it are rewritten by
    // `de_engine_tag_compat` below into a `Vst3` tag carrying
    // `TB303_CLASS_ID_HEX`.
    // Vcs3 left this crate: it is the `Phonix VCS3` VST3, in its own
    // repository. Legacy sessions naming it are rewritten by
    // `de_engine_tag_compat` below into a `Vst3` tag carrying
    // `VCS3_CLASS_ID_HEX`.
    // Mellotron left this crate: it is the `Phonix Mellotron` VST3, in its own
    // repository. Legacy sessions naming it are rewritten by
    // `de_engine_tag_compat` below into a `Vst3` tag carrying
    // `MELLOTRON_CLASS_ID_HEX`.
    // Nebula left this crate: it is the `Phonix Nebula` VST3, in its own
    // repository. Legacy sessions naming it are rewritten by
    // `de_engine_tag_compat` below into a `Vst3` tag carrying
    // `NEBULA_CLASS_ID_HEX`.
    Plexus,
    Chorale,
    // Magma left this crate: it is the `Phonix Magma` VST3, in its own
    // repository. Legacy sessions naming it are rewritten by
    // `de_engine_tag_compat` below into a `Vst3` tag carrying
    // `MAGMA_CLASS_ID_HEX`.
    Pulsar,
    // Aurora left this crate: it is the `Phonix Aurora` VST3, in its own
    // repository. Legacy sessions naming it are rewritten by
    // `de_engine_tag_compat` below into a `Vst3` tag carrying
    // `AURORA_CLASS_ID_HEX`.
    TranceVoice,
    Aria,
    /// Bowed-string / harpsichord physical model (violin/viola/cello/bass + pizz + harpsi).
    Archet,
    // Canon909 was never an instrument: it was one drum kit replayed by an engine
    // of its own. That kit is the drum machine's `RockOpera` kit, so the tag is
    // gone and `de_engine_tag_compat` below rewrites a legacy `"Canon909"` into
    // `Drum`. This is an in-crate RE-MAP, not an extraction, so it has no `Vst3`
    // tag and no row in the class-id list above; the patch is carried over by
    // `session::migrate_legacy_canon909_tracks`.
    // Sampler left this crate: it is the `Phonix Sampler` VST3, in its own
    // repository. Legacy sessions naming it are rewritten by
    // `de_engine_tag_compat` below into a `Vst3` tag carrying
    // `SAMPLER_CLASS_ID_HEX`.
    // Grain left this crate: it is the `Phonix Grain` VST3, in its own
    // repository. Legacy sessions naming it are rewritten by
    // `de_engine_tag_compat` below into a `Vst3` tag carrying
    // `GRAIN_CLASS_ID_HEX`.
    // Spectral left this crate: it is the `Phonix Spectral` VST3, in its own
    // repository. Legacy sessions naming it are rewritten by
    // `de_engine_tag_compat` below into a `Vst3` tag carrying
    // `SPECTRAL_CLASS_ID_HEX`.
    // Orgue left this crate: it is the `Phonix Orgue` VST3, in its own
    // repository. Legacy sessions naming it are rewritten by
    // `de_engine_tag_compat` below into a `Vst3` tag carrying
    // `ORGUE_CLASS_ID_HEX`.
    /// Hosted VST3 instrument plugin. `plugin_path` is the `.vst3` bundle or
    /// DLL path.
    ///
    /// `class_id` is the plugin's VST3 class id as 32 ASCII hex characters, the
    /// same spelling `.vstpreset` headers and DAWproject `deviceID`s use. The
    /// path alone is machine-specific: a session moved to another machine, or
    /// restored from a backup, points at a bundle that is not there. The class
    /// id survives that, so the host can find the plugin again by scanning.
    ///
    /// `#[serde(default)]` because sessions written before this field exists
    /// must keep opening; `None` then means "path only, resolve or fail".
    Vst3 {
        plugin_path: String,
        #[serde(default)]
        class_id: Option<String>,
    },
    /// Hosted CLAP instrument: the `.clap` file and the plugin's id in
    /// it, empty until the plugin is first opened.
    Clap {
        plugin_path: String,
        #[serde(default)]
        plugin_id: String,
    },
    /// Imported audio stem played back from a WAV file. `path` is the source
    /// WAV; the clip's timeline position/length come from its pattern note.
    /// Host-created on audio import, not an Add-Track menu entry.
    AudioClip { path: String },
}

/// Read a track's engine tag, translating the ones that no longer exist.
///
/// `Marteau` and `Souffle` were in-tree engines and are now ordinary scanned
/// VST3s. Every `.aeth` written before their move carries the bare string
/// `"Marteau"` or `"Souffle"`, which the derived reader would reject outright —
/// the whole file, not just the track.
///
/// This cannot be a `#[serde(alias)]`. An alias renames one variant into
/// another of the SAME shape; here a UNIT variant has to become a STRUCT
/// variant, and serde would route the name to `Vst3` and then ask the unit
/// accessor for struct contents, which fails. Aliases handle renames, not
/// promotions. `Polaris` is the proof: it really did carry
/// `#[serde(alias = "NordLead")]` while both names were unit variants, and that
/// alias had to become two rows in the table below when the engine left.
///
/// The legacy piano lands as a Vst3 track with no path and a known class id —
/// exactly the state a project moved between machines arrives in, so it takes
/// the resolution path that already exists instead of needing one of its own.
/// The patch itself is moved by `session::migrate_legacy_marteau_tracks`, which
/// runs after the whole file has parsed.
fn de_engine_tag_compat<'de, D>(d: D) -> Result<EngineTag, D::Error>
where
    D: serde::Deserializer<'de>,
{
    use serde::de::{value::MapAccessDeserializer, value::StrDeserializer, MapAccess, Visitor};

    struct V;
    impl<'de> Visitor<'de> for V {
        type Value = EngineTag;

        fn expecting(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            f.write_str("an engine tag")
        }

        fn visit_str<E: serde::de::Error>(self, s: &str) -> Result<EngineTag, E> {
            for (legacy, hex) in [
                // Both names, one plugin: a session written before the rename
                // says "Marteau", one written after says "Cordis", and either
                // becomes the piano under the id it publishes today.
                ("Marteau", CORDIS_CLASS_ID_HEX),
                ("Cordis", CORDIS_CLASS_ID_HEX),
                ("Souffle", SOUFFLE_CLASS_ID_HEX),
                ("Atmosphera", ATMOSPHERA_CLASS_ID_HEX),
                ("TechnoKick", TECHNO_KICK_CLASS_ID_HEX),
                ("Spectral", SPECTRAL_CLASS_ID_HEX),
                ("Vcs3", VCS3_CLASS_ID_HEX),
                ("Magma", MAGMA_CLASS_ID_HEX),
                ("Harmonizer", HARMONIZER_CLASS_ID_HEX),
                ("Orgue", ORGUE_CLASS_ID_HEX),
                ("Grain", GRAIN_CLASS_ID_HEX),
                ("Mellotron", MELLOTRON_CLASS_ID_HEX),
                ("Aurora", AURORA_CLASS_ID_HEX),
                ("Sampler", SAMPLER_CLASS_ID_HEX),
                ("Tb303", TB303_CLASS_ID_HEX),
                ("Nebula", NEBULA_CLASS_ID_HEX),
                ("Solstice", SOLSTICE_CLASS_ID_HEX),
                ("VP330", VP330_CLASS_ID_HEX),
                ("Strata", STRATA_CLASS_ID_HEX),
                // Two names, one plugin: `NordLead` was a `#[serde(alias)]` on
                // the `Polaris` variant while both were unit variants, and an
                // alias cannot survive the promotion to a struct variant.
                ("Polaris", POLARIS_CLASS_ID_HEX),
                ("NordLead", POLARIS_CLASS_ID_HEX),
            ] {
                if s.eq_ignore_ascii_case(legacy) {
                    return Ok(EngineTag::Vst3 {
                        plugin_path: String::new(),
                        class_id: Some(hex.to_string()),
                    });
                }
            }
            // Canon909 is the one that did NOT leave the crate: its kit was folded
            // into the drum machine, so its tracks become ordinary Drum tracks.
            // Same reason this cannot be a `#[serde(alias)]` as the promotions
            // above — an alias renames, and here `session` still has to move the
            // patch onto a drum project that selects the kit.
            if s.eq_ignore_ascii_case("Canon909") {
                return Ok(EngineTag::Drum);
            }
            // Everything else goes through the derived reader untouched.
            EngineTag::deserialize(StrDeserializer::new(s))
        }

        fn visit_map<A: MapAccess<'de>>(self, map: A) -> Result<EngineTag, A::Error> {
            // The variants that carry data: `Vst3`, `AudioClip`.
            EngineTag::deserialize(MapAccessDeserializer::new(map))
        }
    }

    // Both serde_json and rmp-serde are self-describing, so asking for `any` is
    // legal in each and is what lets one function serve the JSON and the
    // MessagePack halves of the container.
    d.deserialize_any(V)
}
...
impl EngineTag {
    pub fn display_name(&self) -> Cow<'static, str> {
        match self {
            // Kept in lockstep with the engine_catalog() label (drift-guarded by
            // `display_name_matches_catalog_label`), so the mixer, track header,
            // arrangement and preset browser show the SAME name as the Add-Track list.
            EngineTag::Loquace    => "Loquace Talkbox".into(),
            EngineTag::Singer     => "Singer Voice".into(),
            EngineTag::Drum       => "Drum Machine".into(),
            EngineTag::Guitar    => "Electric Guitar".into(),
            EngineTag::Bass      => "Substrate Bass".into(),
            EngineTag::Plexus => "Plexus Drums".into(),
            EngineTag::Chorale => "Chorale Choir".into(),
            EngineTag::Pulsar => "Pulsar Bass".into(),
            EngineTag::TranceVoice => "TranceVoice Choir".into(),
            EngineTag::Aria => "Aria Voice".into(),
            EngineTag::Archet => "Archet Strings".into(),
            EngineTag::Vst3 { plugin_path, .. } => {
                let name = std::path::Path::new(plugin_path)
                    .file_stem()
                    .map(|s| s.to_string_lossy().into_owned())
                    .unwrap_or_else(|| "VST3".to_string());
                Cow::Owned(name)
            }
            EngineTag::Clap { plugin_path, .. } => {
                let name = std::path::Path::new(plugin_path)
                    .file_stem()
                    .map(|s| s.to_string_lossy().into_owned())
...
// ── Add-Track catalog ─────────────────────────────────────────────────────────
//
// Canonical list of all in-tree (non-VST3) engines, with their default
// display name, track kind and GUI accent color. Every Add-Track menu
// / palette in the GUI iterates this single list — there used to be
// SIX copies of the same `(name, EngineTag, TrackKind, color)` array
// scattered across `session_view.rs`, `track_list.rs`, `arrangement_view.rs`,
// `sequencer_app/mod.rs`, `command_palette.rs`, plus implicit filters in
// `matrix_view.rs` — and a new engine showed up in zero, one or all six
// depending on how many were remembered. Now there is exactly one.
//
// The unit test `engine_catalog_covers_every_tag` asserts that every
// in-tree `EngineTag` variant (everything except `Vst3`) appears here,
// so a new variant cannot be added to the enum without also showing
// up in the UI.

/// Static description of one engine for the Add-Track / palette UIs.
#[derive(Debug, Clone)]
pub struct EngineCatalogEntry {
    pub label: &'static str,
    pub tag:   EngineTag,
    pub kind:  TrackKind,
    /// RGB accent color for the track header / GUI chrome.
    pub color: [u8; 3],
    /// Specialty group for the Add-Track menu (finer than `kind`), so 30+
    /// engines are navigable. See `ENGINE_CATEGORIES` for the display order.
    pub category: &'static str,
    /// One-line "what it is", shown dimmed next to the label so the specialty
    /// reads at a glance.
    pub descriptor: &'static str,
}

/// Display order of the specialty categories in the Add-Track menu.
pub const ENGINE_CATEGORIES: [&str; 9] = [
    "Bass & Lead Synths",
    "FM & Wavetable",
    "Pads & Texture",
    "Semi-Modular",
...
pub fn engine_catalog() -> Vec<EngineCatalogEntry> {
    macro_rules! e {
        ($l:expr, $t:ident, $k:ident, $c:expr, $cat:expr, $d:expr) => {
            EngineCatalogEntry { label: $l, tag: EngineTag::$t, kind: TrackKind::$k,
                color: $c, category: $cat, descriptor: $d }
        };
    }
    vec![
        // Bass & Lead Synths
        e!("Pulsar Bass", Pulsar,     Melodic, [255,  60, 200], "Bass & Lead Synths", "Serum-style dubstep bass"),
        e!("Substrate Bass", Bass,       Melodic, [0,   200, 255], "Bass & Lead Synths", "acid / sub / wobble bass"),
        // FM & Wavetable
        // "Strata" is no longer here: it ships as the `Phonix Strata` VST3
        // from its own repository, and the Add-Track menu reads this same
        // table, so the two stay in step by construction.
        // Pads & Texture
        // Physical Models
        e!("Electric Guitar", Guitar,     Melodic, [255, 140,  50], "Physical Models", "electric guitar + amp"),
        e!("Archet Strings", Archet,     Melodic, [214, 158,  78], "Physical Models", "bowed strings (violin family)"),
        // Voices & Choirs
        e!("Aria Voice",  Aria,       Melodic, [235, 170, 120], "Voices & Choirs", "operatic solo voice"),
        e!("Singer Voice", Singer,     Melodic, [180, 100, 220], "Voices & Choirs", "vocal-tract solo voice"),
        e!("Chorale Choir", Chorale,    Melodic, [220, 180, 200], "Voices & Choirs", "MBROLA choir"),
        e!("TranceVoice Choir", TranceVoice, Melodic, [120, 220, 255], "Voices & Choirs", "trance choir / vox"),
        e!("Loquace Talkbox", Loquace,    Melodic, [80,  160, 220], "Voices & Choirs", "talkbox singing formant"),
        // Vintage Keys
        // "Orgue" is no longer here: it ships as the `Phonix Orgue` VST3 from
        // its own repository, and the Add-Track menu reads this same table, so
        // the two stay in step by construction.
        // "Mellotron" is no longer here: it ships as the `Phonix Mellotron`
        // VST3 from its own repository, and the Add-Track menu reads this same
        // table, so the two stay in step by construction.
        // "VP-330" is no longer here: it ships as the `Phonix VP-330` VST3
        // from its own repository, and the Add-Track menu reads this same
        // table, so the two stay in step by construction.
        // Sampling & Granular
        // "Multisampler" is no longer here: it ships as the `Phonix Sampler`
        // VST3 from its own repository, and the Add-Track menu reads this same
        // table, so the two stay in step by construction.
        // "Spectral" is no longer here: it ships as the `Phonix Spectral` VST3
        // from its own repository, and the Add-Track menu reads this same
        // table, so the two stay in step by construction.
        // Drums & Percussion
        e!("Drum Machine", Drum,      Drum,    [220,  80,  80], "Drums & Percussion", "drum machine"),
        e!("Plexus Drums", Plexus,    Drum,    [255,  90, 150], "Drums & Percussion", "generative drums (techno)"),
        // "Canon 909" is no longer here: it was one kit, not an engine, and it is
        // the Drum Machine's Rock Opera kit. The Add-Track menu reads this same
        // table, so the two stay in step by construction.
    ]
}

#[cfg(test)]
mod engine_catalog_tests {
    use super::*;

    /// Polaris left this crate, and it carries TWO legacy names: it shipped as
    /// `NordLead` first and was renamed before the extraction, so `.phx` files
    /// exist with either string. Both have to land on the same hosted plugin.
    ///
    /// `#[serde(alias)]` used to do the rename; the compat reader does both
    /// jobs now, because the target is a struct variant an alias cannot reach.
    #[test]
    fn both_legacy_polaris_names_load_as_the_hosted_plugin() {
        #[derive(serde::Deserialize)]
        struct T {
            #[serde(deserialize_with = "de_engine_tag_compat")]
            engine: EngineTag,
        }
        for name in ["Polaris", "NordLead"] {
            let t: T = serde_json::from_str(&format!("{{\"engine\":\"{name}\"}}")).unwrap();
            assert_eq!(
                t.engine,
                EngineTag::Vst3 {
                    plugin_path: String::new(),
                    class_id: Some(POLARIS_CLASS_ID_HEX.to_string()),
                },
                "legacy tag {name:?} must become the hosted Polaris plugin",
            );
        }
    }

    /// Compile-time-ish guarantee that adding a new variant to
    /// `EngineTag` also adds it to `engine_catalog()`. The match below
    /// is exhaustive on `EngineTag` so the compiler rejects new
    /// variants until you either list them here OR explicitly mark
    /// them as catalog-excluded. `Vst3` is the only legitimate
    /// exclusion (host-discovered, not Add-Track menu material).
    #[test]
    fn every_entry_has_a_valid_category_and_descriptor() {
        for e in engine_catalog() {
            assert!(ENGINE_CATEGORIES.contains(&e.category),
                "'{}' has category '{}' not in ENGINE_CATEGORIES", e.label, e.category);
            assert!(!e.descriptor.is_empty(), "'{}' has no descriptor", e.label);
        }
    }

    /// The Add-Track menu label (engine_catalog) and the app-wide display name
    /// (EngineTag::display_name, used by the mixer, track header, arrangement
    /// and preset browser) must stay identical, or a plugin reads under two
    /// different names depending on where you look. Locks the rename.
    #[test]
    fn display_name_matches_catalog_label() {
        for e in engine_catalog() {
            assert_eq!(e.tag.display_name(), e.label,
                "'{}' display_name() disagrees with its catalog label", e.label);
        }
    }

    #[test]
    fn engine_catalog_covers_every_tag() {
        let cat = engine_catalog();
        // For each tag, decide whether the catalog must include it.
        fn must_be_in_catalog(t: &EngineTag) -> bool {
            match t {
                EngineTag::Vst3 { .. } => false, // host-discovered
                EngineTag::Clap { .. } => false,
                EngineTag::AudioClip { .. } => false, // host-created on audio import
                EngineTag::Loquace
                | EngineTag::Singer
                | EngineTag::Drum
                | EngineTag::Guitar
                ...
                | EngineTag::Chorale
                | EngineTag::Pulsar
                | EngineTag::TranceVoice
                | EngineTag::Aria
                | EngineTag::Archet => true,
            }
        }
        // Enumerate every variant; the match above will fail to
        // compile when EngineTag grows, forcing the author to opt
        // each new variant in (or explicitly out) of the catalog.
        let all_required: &[EngineTag] = &[
            EngineTag::Loquace, EngineTag::Singer,
            EngineTag::Drum,
            EngineTag::Guitar, EngineTag::Bass,
            EngineTag::Plexus,
            EngineTag::Chorale,
            EngineTag::Pulsar,
            EngineTag::TranceVoice, EngineTag::Aria,
            EngineTag::Archet,
        ];
        for t in all_required {
            assert!(must_be_in_catalog(t),
                "tag {t:?} marked as not-in-catalog but appears in required list");
            assert!(cat.iter().any(|e| &e.tag == t),
                "engine_catalog() is missing {t:?} — every new EngineTag MUST be \
                 added to engine_catalog() in src/sequencer/data.rs. This test \
                 exists specifically to catch that drift before it ships.");
        }
        // Also assert no duplicates (avoids a paste bug where the
        // same tag lands twice).
        let mut seen = std::collections::HashSet::new();
        for e in &cat {
            assert!(seen.insert(format!("{:?}", e.tag)),
                "engine_catalog() lists {:?} twice", e.tag);
        }
    }
}
...
pub struct TrackConfig {
    ...
    /// Per-track insert FX chain (up to 4 slots). Empty = no insert FX.
    #[serde(default)]
...
/// Serializable snapshot of one insert-FX slot on a sequencer track.
/// `effect_type` is the `EffectType` discriminant (0 = None/empty).
#[derive(Serialize, Deserialize, Clone, Debug, Default)]
pub struct FxSlotState {
    ...
    /// Sidechain key source track index for keyed effects (Compressor,
    /// MultibandDucker). `None` = self-keyed. Serialized so a `.phx` that
    /// routes e.g. a bass into a master-bus ducker round-trips.
    #[serde(default)]
    pub sidechain_src: Option<u16>,
    /// Sub-slots when this slot holds a Multi-FX. Flat, one level deep, and
    /// `#[serde(default)]` so sessions written before Multi-FX still load.
    #[serde(default)]
    pub nested: Vec<FxSlotState>,
    /// The chains beside the first, the gains and zones, the selector and
    /// the macros, when the slot is a rack.
    #[serde(default)]
    pub rack: RackState,
}

/// A macro's hold on one parameter in a rack's chains: the macro from 0
/// to 1 drives the parameter from `lo` to `hi`.
...
/// A Multi-FX slot's rack: chains in parallel after the first (which is
/// the slot's `nested`), each with a gain and the selector zone it plays
/// in, the selector, eight macros and what they hold.
#[derive(Serialize, Deserialize, Clone, Debug)]
...
impl RackState {
    ...
    /// Chains in all: the first and the ones beside it.
    pub fn chain_count(&self) -> usize { self.chains.len() + 1 }
}

impl Default for RackState {
    fn default() -> Self {
        Self { chains: Vec::new(), gains: Self::default_gains(), zones: Self::default_zones(), selector: 0, macros: [0.0; 8], maps: Vec::new() }
    }
}
...
pub struct MixerState {
    ...
    /// Master bus insert FX chain (mastering chain), up to 8 slots.
    #[serde(default)]
    pub master_fx_slots: Vec<FxSlotState>,
    ...
    #[serde(default)]
    pub mix_fx_slots: Vec<FxSlotState>,
...
pub struct Session {
    ...
    #[serde(default)]
    pub groove: Option<crate::groove::GrooveTemplate>,
...
pub struct NamedGroove {
    pub name: String,
    pub template: crate::groove::GrooveTemplate,
}
...
impl Session {
    ...
    pub fn remap_track_refs_after_delete(&mut self, deleted: usize) {
        ...
        for t in &mut self.tracks {
            ...
            for slot in &mut t.fx_slots {
                fix(&mut slot.sidechain_src);
            }
        }
    }
